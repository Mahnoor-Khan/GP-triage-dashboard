...
export default function Sidebar({ setLogin }) {
  ...
  return (
    <div className="bg-[#E5E5E5] flex flex-col gap-[370px] justify-between py-8 h-full">
          ...
          {routes.map((route) => (
            <Link to={route.to}>
              <div
                ...
                onClick={() => handleMenuItemClick(route.id)}
              >
                <img
                  src={
                    activeMenuId === route.id
                      ? route.activeIcon
                      : route.inactiveIcon
                  }
                  alt="_icon"
                  className={`${
                    activeMenuId === route.id ? "bg-[#d6dbdf]" : ""
                  } py-4 pl-8 pr-1 rounded-r-md`}
                />
                <p
                  className={` ${
                    activeMenuId === route.id
                      ? "text-[#0E72B7]"
                      : "text-[#767D88]"
                  } text-base font-medium leading-normal font-dm-sans`}
                >
                  {route.route}
                </p>
              </div>
      ...
      <div className="flex px-4 items-center gap-5">
        <div className="flex gap-2 items-center">
          ...
          <div className="flex flex-col gap-1">
            <p className="text-[#06152B] text-xs leading-normal font-dm-sans font-medium">
              Dr. John Smith
            </p>
            <p className="text-[#767d88] text-[10px] leading-normal font-dm-sans">
              GP Partner
            </p>
          </div>
        </div>
